...
TEST_CASE("FluidsProgram creates expected action sequence", "[fluids][program]")
{
    core::master::plugin::KV kv = {{"time_scheme", "fe"}, {"fe_iters", "3"}, {"dx", "1"},
                                   {"dy", "1"},           {"dz", "1"},       {"rho", "1"},
                                   {"nu", "1e-3"},        {"Cs", "0.16"}};
    core::master::RunContext rc{};
    fluids::FluidsProgram prog(kv, rc);

    auto plan = prog.plan_step(1.0);
    // FluidsProgram returns a single loop action (plus optional one-time init if configured).
