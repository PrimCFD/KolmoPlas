...
def main(path: str = "build-scaling/results_scaling.csv",
         outdir: str = "build-scaling/scaling_reports") -> None:
    """
    Post-process strong-scaling CSVs into a cleaned summary + plots.

    Parameters
    ----------
    path : str
        Path to the raw CSV (as produced by your scaling harness).
    outdir : str
        Output directory for the cleaned CSV and PNG plots.
    """
    csv = Path(path)
    if not csv.exists():
    ...
    df["nice_factorization"] = [
        is_nice_factorization(int(n), int(r))
        if pd.notna(n) and pd.notna(r) else False
        for n, r in zip(df.get("grid_n", []), df.get("ranks", []))
    ]

    agg = (
        df.groupby(["grid_n", "ranks"], dropna=True)
        .agg(
    ...
    for g, gdf in res.groupby("grid_n"):
        ...
        plt.ylabel("Speedup (vs min ranks)")
...
def cli() -> None:
    parser = argparse.ArgumentParser(
        description="Post-process strong-scaling CSV into a cleaned summary + plots."
    )
    parser.add_argument(
        "--csv",
        dest="csv",
        default="build-scaling/results_scaling.csv",
        help="Input scaling CSV file (default: %(default)s).",
    )
    parser.add_argument(
        "--out-dir",
        dest="out_dir",
        default="build-scaling/scaling_reports",
        help="Output directory for cleaned CSV and plots (default: %(default)s).",
    )

    args = parser.parse_args()
    main(path=args.csv, outdir=args.out_dir)
