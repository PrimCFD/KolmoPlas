...
void ApplyBCs::execute(const MeshTileView& tile, FieldCatalog& fields, double)
{
    ...
    if (fields.contains("u"))
    {
        ...
        fu = std::make_unique<Field<double>>(
            ...
            std::array<int, 3>{vu.extents[0], vu.extents[1], vu.extents[2]}, ng_mesh);
    ...
    if (fields.contains("v"))
    {
        ...
        fv = std::make_unique<Field<double>>(
            ...
            std::array<int, 3>{vv.extents[0], vv.extents[1], vv.extents[2]}, ng_mesh);
    ...
    if (fields.contains("w"))
    {
        ...
        fw = std::make_unique<Field<double>>(
            ...
            std::array<int, 3>{vw.extents[0], vw.extents[1], vw.extents[2]}, ng_mesh);
    ...
    if (fields.contains("p"))
    {
        ...
        fp = std::make_unique<Field<double>>(
            ...
            std::array<int, 3>{vp.extents[0], vp.extents[1], vp.extents[2]}, ng_mesh);
    ...
    for (const auto& f : faces)
    {
        ...
        // NOTE: the kernels in Boundary.hpp already fill *all* ghost layers l=1..ng,
        // so we only need to call them once. The older "reps = ng" loop was defensive.
        const int reps = 1;
        ...
        for (int r = 0; r < reps; ++r)
        {
            ...
            if (have("p") && sp)
            {
                ...
                // For a scalar, "mirror" is equivalent to zero-gradient. If user ever
                // writes bc.p.*: mirror, treat it as NeumannZero.
                if (op == BCOp::Mirror)
                    op = BCOp::NeumannZero;
